...
### 实验环境要求
- **硬件环境**：
  - Nuclei DDR200T FPGA开发板
  - Hummingbird Debugger Kit 调试器
  - 电脑（Windows 或 Linux 系统）
  - USB 数据线、JTAG 连接线、电源线

- **软件环境**：
  - Vivado 工具（用于 FPGA 固件烧录）
  - 串口通信工具（Tera Term、minicom 等，windows可使用toolchain中附带的串口工具）
...
| 器材名称 | 数量 | 备注 |
|---------|------|------|
| DDR200T 开发板 | 1 | Nuclei FPGA 开发板板 |
| Hummingbird Debugger Kit | 1 | 调试器套件 |
| 12V 电源适配器 | 1 | 开发板供电 |
| USB  数据线 | 1 | FPGA 固件烧录 |
| JTAG 数据线 | 1 | MCU 烧录与串口通信 |
...
│   └── syn           # 综合文件
├── README.md
└── toolchain          # 此处对应自己的系统下载对应版本后改名
    ├── SSCOM32.exe    # Windows 串口工具 （Linux无此文件）
    ├── env-windows    # Windows 环境配置工具 （Linux无此文件夹）
    ├── build-tool     # 构建工具 （Linux无此文件夹）
    ├── gcc           # RISC-V 工具链
...
2. 打开 Vivado 工具，进入 Hardware Manager，使用open target -> auto connect连接到设备。
![alt text](Snipaste_2026-03-26_10-02-40.png)
...
![alt text](Snipaste_2026-03-26_10-22-42.png)
7. 确认烧录，等待完成
...
### 实验目标
1. 掌握裸机程序的编译与运行流程
2. 了解 Hummingbird E603 SoC 的基本功能特性
3. 通过 cpuinfo 示例程序查看 SoC 配置信息
4. 理解 SoC 支持的各种硬件功能模块
...
#### 步骤 2：进入裸机程序目录
使用同一终端进入 cpuinfo 示例程序目录：
```bash
cd e603_hbird-main/nuclei-sdk/application/baremetal/cpuinfo
```
...
#### 步骤 5：运行验证
1. 在串口工具中查看程序输出信息
2. 按下 MCU RESET 键可以重新运行程序

### 实验内容分析

#### cpuinfo 程序功能
cpuinfo 程序会读取并显示 Hummingbird E603 SoC 的详细配置信息，包括：
- CPU 名称、版本和 ISA 扩展
- 内存子系统配置（ILM、DLM）
- 缓存配置（ICACHE、DCACHE）
- 中断控制器（ECLIC、PLIC）
...
-----Nuclei RISC-V CPU Configuration Information-----
         MARCHID: 0x...
          MIMPID: 0x...
             ISA: RV32 I M A F D C NICE
            MCFG: ECLIC PLIC NICE ILM DLM ICACHE DCACHE ...
             ILM: 64 KB
             DLM: 64 KB
          ICACHE: 16 KB(set=64,way=4,lsize=16,ecc=1)
          DCACHE: 16 KB(set=64,way=4,lsize=16,ecc=1)
             TLB: MainTLB(set=8,way=4,entry=64,ecc=1) ITLB(entry=16) DTLB(entry=16)
         IREGION: 0x60000000 1 MB
           ECLIC: VERSION=0x1 NUM_INTERRUPT=87 CLICINTCTLBITS=8 MTH=1 NLBITS=2
-----End of Nuclei CPU INFO-----
```

### SoC 功能概览总结
通过 cpuinfo 程序的输出，实验人员可以了解到该 SoC 支持以下功能：

#### 1. 处理器核心特性
- **RISC-V RV32IMAFDC** 指令集架构
- **NICE**（Nuclei Instruction Custom Extension）自定义指令扩展
- **硬件乘除法器**（M扩展）
- **单精度浮点单元**（F扩展）
- **双精度浮点单元**（D扩展）
- **原子操作**（A扩展）
...
#### 2. 内存子系统
- **ILM**（Instruction Local Memory）：指令本地内存，用于高速指令访问
- **DLM**（Data Local Memory）：数据本地内存，用于高速数据访问
- **ICACHE**：指令缓存，提升指令读取性能
- **DCACHE**：数据缓存，提升数据访问性能
- **ECC**（Error Correction Code）：错误校验与纠正功能

#### 3. 中断系统
- **ECLIC**（Enhanced Core Local Interrupt Controller）：增强型核心本地中断控制器
- **PLIC**（Platform Level Interrupt Controller）：平台级中断控制器
- 支持多级中断优先级和抢占

#### 4. 系统功能
- **IREGION**：系统寄存器区域，包含各种配置寄存器
- **TLB**（Translation Lookaside Buffer）：地址转换缓存
- **SMP**（Symmetric Multiprocessing）：对称多处理支持（如配置）
- **安全机制**：支持多种安全运行模式

#### 5. 外设接口
- **FIO**（Fast IO）：快速IO接口
- **PPI**（Peripheral Private Interface）：外设私有接口
- **定时器**：系统定时器和看门狗
- **GPIO**：通用输入输出接口
...
|------|----------|----------|------|
| 环境准备 | 串口工具正常连接 | | |
| 编译程序 | 生成 cpuinfo.elf 文件 | | |
| 程序上传 | 上传成功提示 | | |
| 运行验证 | 串口显示 SoC 配置信息 | | |

### 实验意义
通过本实验，实验人员可以：
1. **验证开发环境**：确认工具链、调试器和串口通信正常工作
2. **了解 SoC 能力**：全面掌握 Hummingbird E603 SoC 的硬件功能
3. **建立基准认知**：为后续 RT-Thread 等操作系统实验打下基础
4. **调试参考**：cpuinfo 输出可作为系统调试的重要参考信息

### 扩展探索：其他裸机程序示例
在 `e603_hbird-main/nuclei-sdk/application/baremetal/` 目录下，还提供了多个裸机程序示例，可以帮助用户深入理解 SoC 的各项功能：

#### 1. 基础示例
- **helloworld**：最简单的 "Hello World" 程序，验证基本编译和运行流程
- **empty**：空程序模板，可作为自定义程序的起点
- **cpuinfo**：本实验使用的 SoC 信息查询程序

#### 2. 外设功能演示
- **demo_gpio**：GPIO 控制示例，学习如何控制 LED、读取按键等
- **demo_timer**：定时器使用示例，掌握系统定时器和中断处理
- **demo_clint_timer**：CLINT（Core Local Interruptor）定时器示例
- **demo_eclic**：ECLIC（Enhanced Core Local Interrupt Controller）中断控制器示例

#### 3. 高级功能探索
- **demo_nice**：NICE（Nuclei Instruction Custom Extension）自定义指令扩展示例
- **demo_pmp**：PMP（Physical Memory Protection）物理内存保护示例
- **lowpower**：低功耗模式示例，学习如何进入和退出低功耗状态

#### 4. 性能基准测试
- **benchmark**：性能基准测试套件
  - **benchmark/coremark**：CoreMark 基准测试，评估处理器核心性能
  - **benchmark/dhrystone**：Dhrystone 基准测试，评估整数运算性能

#### 使用指导
要运行这些示例程序，可以按照以下步骤操作：
1. 进入相应的程序目录：
   ```bash
   cd e603_hbird-main/nuclei-sdk/application/baremetal/[程序名称]
   ```
2. 编译程序：
   ```bash
   make clean SOC=evalsoc CORE=ux600fd BOARD=nuclei_fpga_eval all
...
#### 学习建议
- 从简单的 `helloworld` 开始，确保基本流程正确
- 逐步尝试 `demo_gpio` 和 `demo_timer`，理解外设控制
- 通过 `demo_nice` 了解 SoC 的自定义指令扩展能力
- 使用 `benchmark` 程序评估系统性能
- 参考这些示例的源代码，学习如何编写自己的裸机应用程序

这些示例程序覆盖了 Hummingbird E603 SoC 的主要功能模块，为用户提供了丰富的学习资源和开发参考。

## 实验二：RT-Thread 5.2 系统编译与运行
...
#### 步骤 1：进入实验目录
按照实验一的步骤完成环境配置，然后进入 RT-Thread 5.2 示例目录：
```bash
cd e603_hbird-main/nuclei-sdk/application/rtthread_5_2/msh
```
